...
public class DrugSearchPropertiesBean {
	...
	private void LoadPropertiesFile() {
		...
		try {
			...
			e.printStackTrace(); 
			logger.logError("LoadPropertiesFile", -1, String.format("Can't load Properties file:[%s] with Exception:[%s]", "DrugSearchProperties.properties",e.getMessage()),e);
			mobjProp = null ;
		}finally{
			if (input!=null)
				try {
	...
	public String GetFDASearchPattern(){
		...
		String strResult = mobjProp.getProperty("Search.Drug.pattern.prefix", null) ;
